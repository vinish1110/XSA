...
var xsuaa_service_1 = require("./legacy/xsuaa-service");
var xsuaa_service_2 = require("./xsuaa-service");
/**
...
function serviceToken(service, options) {
    return __awaiter(this, void 0, void 0, function () {
        var opts, serviceCredentials, xsuaa, cachedToken, token, err_1;
        return __generator(this, function (_a) {
            switch (_a.label) {
                case 0:
                    opts = __assign({ useCache: true, enableCircuitBreaker: true }, options);
                    service = environment_accessor_1.resolveService(service);
                    serviceCredentials = service.credentials;
                    xsuaa = multiTenantXsuaaCredentials(options);
                    if (opts.useCache) {
                        cachedToken = client_credentials_token_cache_1.clientCredentialsTokenCache.getGrantTokenFromCache(xsuaa.url, serviceCredentials.clientid);
                        if (cachedToken) {
                ...
                case 1:
                    _a.trys.push([1, 3, , 4]);
                    return [4 /*yield*/, xsuaa_service_2.getClientCredentialsToken(service, options === null || options === void 0 ? void 0 : options.userJwt, options)];
                case 2:
                    token = _a.sent();
                    if (opts.useCache) {
                        client_credentials_token_cache_1.clientCredentialsTokenCache.cacheRetrievedToken(xsuaa.url, serviceCredentials.clientid, token);
                    }
                    return [2 /*return*/, token.access_token];
                case 3:
                    err_1 = _a.sent();
                    throw new util_1.ErrorWithCause("Could not fetch client credentials token for service of type \"" + service.label + "\".", err_1);
...
function userApprovedServiceToken(userJwt, service, options) {
    return __awaiter(this, void 0, void 0, function () {
        ...
        return __generator(this, function (_a) {
            ...
            xsuaa = multiTenantXsuaaCredentials({ userJwt: userJwt });
...
function jwtBearerToken(userJwt, service, options) {
    return __awaiter(this, void 0, void 0, function () {
        var resolvedService, opts;
        return __generator(this, function (_a) {
            resolvedService = environment_accessor_1.resolveService(service);
            opts = __assign({ enableCircuitBreaker: true }, options);
            return [2 /*return*/, xsuaa_service_2.getUserToken(resolvedService, userJwt, opts)];
        });
    });
}
...
function multiTenantXsuaaCredentials(options) {
    ...
    var xsuaa = options.xsuaaCredentials
        ? __assign({}, options.xsuaaCredentials) : environment_accessor_1.getXsuaaServiceCredentials(options.userJwt);
    if (options.userJwt) {
        var decodedJwt = typeof options.userJwt === 'string'
            ? jwt_1.decodeJwt(options.userJwt)
            : options.userJwt;
        if (!decodedJwt.iss) {
