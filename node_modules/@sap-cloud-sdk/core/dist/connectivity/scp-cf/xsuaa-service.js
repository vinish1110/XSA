...
function wrapInCircuitBreaker(fn) {
    return function () {
        ...
        return (_a = getCircuitBreaker()).fire.apply(_a, __spreadArray(__spreadArray([fn], parameters), [{
...
function getSubdomainAndZoneId(jwt) {
    ...
    var zoneId = null;
    if (jwt) {
        var jwtPayload = typeof jwt === 'string' ? jwt_1.decodeJwt(jwt) : jwt;
        if (jwtPayload.iss) {
            subdomain = subdomain_replacer_1.parseSubdomain(jwtPayload.iss);
        }
        if (jwtPayload.zid) {
...
function getClientCredentialsToken(service, userJwt, options) {
    ...
    var serviceCredentials = environment_accessor_1.resolveService(service).credentials;
